...
class search
{
	...
	public function jobseekerData() {
		$this->db->execute("SELECT * FROM `tb_jobseeker` WHERE jobseeker_id='$_SESSION[jobseeker_id]' OR social_id='$_SESSION[FBID_JS]' limit 1");
		return $this->db->getResults();
	...
	///update job views function
	public function updatePostJobsViews(){
		$this->db->execute("UPDATE tb_post_job SET  job_views=job_views+1  WHERE post_job_id = '$_SESSION[job_id]'");
	}
	//for footer
