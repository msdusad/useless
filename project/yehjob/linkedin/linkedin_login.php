...
if ($success) {
  ...
  try {
    $sql = "SELECT * FROM tb_jobseeker WHERE jobseeker_email = '$user->emailAddress' LIMIT 1";
    ...
    if ($num > 0) {
      ...
      $sql1 = "INSERT INTO `tb_jobseeker`(`linkedin_id`,`fname`,`lname`,`jobseeker_email`,`register_date`,`social_type`) VALUES ('$user->id','$user->firstName','$user->lastName','$user->emailAddress',Now(),'Linkedin')";
      $db->execute($sql1);
      $result1 = $db->LastId();
      if ($result1 > 0) {
        $sql2 = "SELECT * FROM tb_jobseeker WHERE jobseeker_email = '$user->emailAddress' LIMIT 1";
        $seeker_res1 = $db->execute($sql2);
        $result1 = $db->getResult($seeker_res1);
        $_SESSION['name'] = $user->formattedName;
        $_SESSION['email'] = $user->emailAddress;
        $_SESSION['picture'] = $user->pictureUrl;
        $_SESSION['jobseeker_id'] = $result1['jobseeker_id'];
...
if(!empty($_SESSION['job_id'])){
  die("<script>location.href = 'http://yehjob.com/job_seeker/details.php';</script>");
}else{
  die("<script>location.href = 'http://yehjob.com/job_seeker/';</script>");
}
